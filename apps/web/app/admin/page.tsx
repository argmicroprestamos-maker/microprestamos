...
type WhatsAppDocument = { type: string; file_name?: string | null; mime_type?: string | null; url?: string | null };
type WhatsAppApplication = { id: string; full_name: string; dni_masked?: string | null; phone_masked?: string | null; birth_date?: string; address?: string; email?: string | null; employment_status: string; employment_detail?: string | null; monthly_income: number; requested_amount: number; contact_1?: EmergencyContact; contact_2?: EmergencyContact; documents_received: number; documents?: WhatsAppDocument[]; cbu_masked?: string | null; holder_name?: string; state: string; created_at: string };
type Action = { kind: 'decision' | 'whatsapp-decision' | 'disburse' | 'payment'; id: string };
...
export default function Admin() {
  ...
  const [summary, setSummary] = useState<Summary | null>(null); const [applications, setApplications] = useState<Application[]>([]); const [whatsappApplications, setWhatsappApplications] = useState<WhatsAppApplication[]>([]); const [loans, setLoans] = useState<Loan[]>([]);
  ...
  async function refresh(accessToken = token) { const [nextSummary, nextApplications, nextWhatsappApplications, nextLoans] = await Promise.all([get('/summary', accessToken), get('/applications', accessToken), get('/whatsapp-applications', accessToken), get('/loans', accessToken)]); setSummary(nextSummary); setApplications(nextApplications.applications ?? []); setWhatsappApplications(nextWhatsappApplications.applications ?? []); setLoans(nextLoans.loans ?? []); }
  ...
  async function submitAction(event: FormEvent) {
    ...
    if (action.kind === 'whatsapp-decision') { path = `/whatsapp-applications/${action.id}/decision`; body = { decision, reason }; }
  ...
  function logout() { supabase?.auth.signOut(); setSummary(null); setApplications([]); setWhatsappApplications([]); setLoans([]); setToken(''); setAction(null); }
  const selectedWhatsapp = action?.kind === 'whatsapp-decision' ? whatsappApplications.find(item => item.id === action.id) : null;
  ...
  return <main className="shell"><nav className="nav"><div className="brand"><span className="mark">MP</span> Panel MicroPréstamos</div><a href="/">Volver al sitio</a></nav><section style={{ padding: '40px 0', maxWidth: 1100, margin: 'auto' }}><h1>Operaciones</h1><p style={{ color: '#64748b' }}>Las decisiones, desembolsos y pagos se registran con actor y auditoría. No se inicia una transferencia bancaria desde este panel.</p>
    {!summary ? <form className="card" onSubmit={login} style={{ display: 'grid', gap: 14, maxWidth: 420 }}><label>Correo<input required type="email" value={email} onChange={e => setEmail(e.target.value)} style={inputStyle} /></label><label>Contraseña<input required type="password" value={password} onChange={e => setPassword(e.target.value)} style={inputStyle} /></label><button className="cta" disabled={loading} style={buttonStyle}>{loading ? 'Ingresando…' : 'Ingresar'}</button>{error && <p role="alert" style={errorStyle}>{error}</p>}</form> : <>
      ...
      {action && <form className="card" onSubmit={submitAction} style={{ display: 'grid', gap: 12, margin: '24px 0' }}><h2>{action.kind === 'decision' || action.kind === 'whatsapp-decision' ? 'Decidir solicitud' : action.kind === 'disburse' ? 'Registrar desembolso manual' : 'Registrar pago'}</h2>
        {selectedWhatsapp && <div style={{ background: '#f8fafc', borderRadius: 8, padding: 14, lineHeight: 1.6 }}><strong>{selectedWhatsapp.full_name}</strong><br />Nacimiento: {selectedWhatsapp.birth_date || 'sin informar'} · Email: {selectedWhatsapp.email || 'sin informar'}<br />Domicilio: {selectedWhatsapp.address || 'sin informar'}<br />Empleo: {selectedWhatsapp.employment_status || 'sin informar'}{selectedWhatsapp.employment_detail ? ` — ${selectedWhatsapp.employment_detail}` : ''} · Ingreso: {money(selectedWhatsapp.monthly_income)}<br />Contacto 1: {selectedWhatsapp.contact_1?.name || 'sin informar'} ({selectedWhatsapp.contact_1?.relationship || '—'}) {selectedWhatsapp.contact_1?.phone || ''}<br />Contacto 2: {selectedWhatsapp.contact_2?.name || 'sin informar'} ({selectedWhatsapp.contact_2?.relationship || '—'}) {selectedWhatsapp.contact_2?.phone || ''}<br />Cuenta: {selectedWhatsapp.holder_name || 'sin titular'} · CBU {selectedWhatsapp.cbu_masked || 'sin informar'}<br />Documentos: {(selectedWhatsapp.documents || []).map(document => document.url ? <a key={document.type} href={document.url} target="_blank" rel="noreferrer" style={{ marginRight: 12 }}>{document.type.replace(/_/g, ' ')}</a> : <span key={document.type} style={{ marginRight: 12 }}>{document.type.replace(/_/g, ' ')} (no disponible)</span>)}<br /><strong>Solicita {money(selectedWhatsapp.requested_amount)}</strong></div>}
        {(action.kind === 'decision' || action.kind === 'whatsapp-decision') && <><label>Decisión<select value={decision} onChange={e => setDecision(e.target.value)} style={inputStyle}><option value="approved">Aprobar</option><option value="rejected">Rechazar</option></select></label><label>Motivo<input required minLength={3} value={reason} onChange={e => setReason(e.target.value)} style={inputStyle} /></label></>}
      ...
      <Table title="Solicitudes por WhatsApp" empty="No hay solicitudes por WhatsApp todavía." headers={['Cliente', 'Empleo', 'Ingreso', 'Monto', 'Documentos', 'Estado', 'Acción']} rows={whatsappApplications.map(a => [<span key={a.id}><strong>{a.full_name}</strong><br /><small>{a.dni_masked} · {a.phone_masked} · CBU {a.cbu_masked}</small></span>, <span key={`${a.id}-job`}>{a.employment_status || 'sin informar'}{a.employment_detail ? <><br /><small>{a.employment_detail}</small></> : null}</span>, money(a.monthly_income), money(a.requested_amount), `${a.documents_received}/3`, a.state.replace(/_/g, ' '), a.state === 'ready_for_review' ? <button key={`${a.id}-review`} onClick={() => setAction({ kind: 'whatsapp-decision', id: a.id })}>Revisar</button> : '—'])} />
