      ...
      Column(Modifier.padding(padding).padding(24.dp).fillMaxSize().verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        ...
        when (step) {
          ...
          Step.SIMULATION -> {
            ...
            Text("Oferta informativa", style = MaterialTheme.typography.headlineSmall); Field("Monto solicitado (ARS)", amount, keyboard = true) { amount = it.filter(Char::isDigit) }
            ...
            Button(enabled = !busy && selected != null && session != null, onClick = { run { val current = requireNotNull(session); SupabaseGateway.createApplication(current, amount.toDoubleOrNull() ?: 0.0, requireNotNull(selected)); applications = SupabaseGateway.applications(current); loans = SupabaseGateway.loans(current); step = Step.STATUS } }, modifier = Modifier.fillMaxWidth()) { Text(if (busy) "Enviando…" else "Enviar solicitud") }
          }
